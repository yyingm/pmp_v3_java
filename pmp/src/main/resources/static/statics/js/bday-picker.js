    ...
    return this.each(function() {
      ...
      var $fieldset = $("<fieldset class='birthday-picker'></fieldset>"),
        //  $year = $("<select class='birth-year' name='birth[year]'></select>"),
          ...
          $month = $("<select class='birth-month tg_select_class' name='birth[month]'></select>"),
          $day = $("<select class='birth-day tg_select_class' name='birth[day]'></select>");
      ...
      if (settings["dateFormat"] == "bigEndian") {
        ...
        if($(".birthday-picker").parent().attr("id")=="month_end_option"){
        	$fieldset.append("至").append($month).append("月").append($day).append("日");
        	//console.log($(".birthday-picker").parent().attr("id"));
        	//console.log($(".birthday-picker").parent().attr("id")=="month_end_option");
        }else{
        	$fieldset.append($month).append("<span style='margin-right:4px;'>月</span>").append($day).append("日");
        	console.log($("fieldset").parent().attr("id"));
        }
        if (tabindex != null) {
      ...
      if (settings["placeholder"]) {
       // $("<option value='0'>Year:</option>").appendTo($year);
        $("<option value='0'>请选择月份</option>").appendTo($month);
        $("<option value='0'>请选择日期</option>").appendTo($day);
      }
     ...
     //   $year.val(date.getFullYear());
      ...
      $fieldset.change(function() {
        ...
        var todayDate = new Date(),
            ...
            actMaxDay = (new Date($year, selectedMonth, 0)).getDate(),
